...
def fetch_url(url, max_retries=3, delay=2):
    for attempt in range(max_retries):
        try:
            ...
            logging.warning(f"Request error: {e}. Attempt {attempt + 1} of {max_retries}. Retrying in {delay} seconds...")
            time.sleep(delay)
...
def book_extraction(isbn):
    ...
    logging.info(f"Fetching URL: {url}")  # Log the URL for debugging
    response = fetch_url(url)  # Send a GET request to the URL
    if response:
        ...
        if script_tag:
            ...
            logging.warning(f"No #__NEXT_DATA__ script tag found for ISBN: {isbn}")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        future_to_isbn = {executor.submit(book_extraction, isbn): isbn for isbn in isbns[:10]} # taking only first 10 inputs 
        ...
        for future in as_completed(future_to_isbn):
            ...
            try:
                ...
                logging.error(f"Error fetching details for ISBN {isbn}: {exc}")
    ...
    logging.info(f"Book details saved to {output_file_path}")  # Print a confirmation message
